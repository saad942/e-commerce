...
function NavBar() {
  ...
  const makePayment = async () => {
    try {
      const stripe = await loadStripe("pk_test_51Pc9A3RoPpKdTVswiQNShWXlx6aE2oN3uXDzSUxGUWLIbjplcy01QLhL2x4ba22gafs43W4w2Aeo442y1nOlaXpj00mOBHEnbb");

      const response = await axios.post("http://localhost:8081/create-checkout-session", {
        cart
      });
      ...
      <div className="buttons-container">
        ...
        <Modal show={showModal} onHide={handleCloseModal} size="lg">
          ...
          <Modal.Footer className="justify-content-center">
            <button className="btn btn-success" onClick={makePayment}>Payment</button>
          </Modal.Footer>
        </Modal>
      </div>
    </div>
